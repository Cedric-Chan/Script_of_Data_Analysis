# 无监督的数据挖掘算法（数据集中未包含因变量y值），不告诉机器评判标准，只是根据相似度进行分类
import pandas as pd
import numpy as np  
...
def k_silhouette(X, clusters):
    ...
    plt.show()
# 自定义函数的调用
k_silhouette(X, 15)           # K=3时轮廓系数最大，且最接近1，即为最佳K值
...
#——————————————————————————————————————————————iris数据集的聚类实例————————————————————————————————————————————————————
iris = pd.read_csv(r'C:\Users\123\Desktop\iris.csv')
iris.head()                                     # 自变量都是长度单位，不存在量纲上的差异，不需要标准化处理
X = iris.drop(labels = 'Species', axis = 1)     # 提取出自变量数据
# 构建Kmeans模型
...
# 绘制散点图与原始数据做对比，直观显示聚类效果
import seaborn as sns
import pygal

centers = kmeans.cluster_centers_               # 三个簇的簇中心
# 绘制聚类效果的散点图
...
#——————————————————————————————————————————————NBA球员数据集的聚类实例————————————————————————————————————————————————————
players = pd.read_csv(r'C:\Users\123\Desktop\players.csv')
players.head()                  # 自变量量纲不同，需进行标准化处理
...
plt.show()                      # 散点图无法看得分类，需用三种方法确定最佳K值

# 使用拐点法选择最佳的K值（定义在前面）
k_SSE(X, 15)                    # （3,4,5左右）
# 使用轮廓系数选择最佳的K值（定义在前面）
k_silhouette(X, 15)             # （K=2轮廓系数最大）
# 使用间隙统计量选择最佳的K值（定义在前面）
gap_statistic(X, B = 20, K=range(1, 16))  # 综上方法，最佳K值定为3
...
# 构建空列表，用于存储三个簇的簇中心
centers = []
for i in players.cluster.unique():      # 因对原数据做了标准化处理，故不可直接调用cluster_center,需用for循环找到原始簇中心
