# Logistic回归属于非线性模型，专门用来解决二分类的离散问题，可用混淆矩阵、ROC曲线、K-S曲线进行评估
import pandas as pd
import numpy as np
...
sports = pd.read_csv(r'C:\Users\123\Desktop\Run or Walk.csv')
predictors = sports.columns[4:]            # 提取出所有自变量名称
X = sports.ix[:,predictors]                # 构建自变量矩阵
y = sports.activity                        # 提取y变量值（bool因变量为是否在运动）
X_train, X_test, y_train, y_test = model_selection.train_test_split(X, y, test_size = 0.25, random_state = 1234)

sklearn_logistic = linear_model.LogisticRegression()       # 调用sklearn子模块linear_model的LogisticRegression类
sklearn_logistic.fit(X_train, y_train)     # 利用训练集建模
print(sklearn_logistic.intercept_, sklearn_logistic.coef_) # 返回模型的各个参数（截距和系数）
# （Xβ=4.36+0.49accx+6.86accy-2.45accz-0.01gyx-0.16gyy+0.13gyz）

sklearn_predict = sklearn_logistic.predict(X_test)         # 模型预测
pd.Series(sklearn_predict).value_counts()                  # 预测结果统计(单凭统计数据无法确定是否预测准确，需要进一步定量评估)

# 评估法一：混淆矩阵
from sklearn import metrics
cm = metrics.confusion_matrix(y_test, sklearn_predict, labels = [0,1])
cm                                         # 返回值即为混淆矩阵，行为实际状态，列为预测状态
...
import matplotlib.pyplot as plt
sns.heatmap(cm, annot = True, fmt = '.2e',cmap = 'GnBu')   # 绘制热力图
plt.show()                                 # 颜色越深所处的样本量越多

# 评估法二：ROC曲线
import matplotlib.pyplot as plt
y_score = sklearn_logistic.predict_proba(X_test)[:,1]      # y得分为模型预测正例的概率，非实际的预测值
...
# 将数据集拆分为训练集和测试集
X_train, X_test, y_train, y_test = model_selection.train_test_split(X, y, test_size = 0.25, random_state = 1234)
# 为训练集和测试集的X矩阵添加常数列1（使用了Logit类，不调用add_constant给X增加常数列1，将不会拟合模型的截距项）
